...
#-----------------------------------------------------------------------------------------------------------------------------------------------------
# [修正履歴]
# V-001      : 2025/04/20                 Gamer-Iris   新規作成
# 
######################################################################################################################################################
*/
...
public class AdminCommandHandler implements CommandExecutor {
    ...
    private boolean handleBan(CommandSender sender, String[] args) {
        ...
        if ((playerSender == null || !hasPermission(sender, "greetmate.command.greetban")
                ...
                || !validateArgsLength(sender, args, 2, "§e[GreetingPlugin] 使用法: /greetban <player> <reason>"))
            return true;
        ...
        sender.sendMessage("§a[GreetingPlugin] " + playerName + " をBAN登録しました。");
        LogWriter.writeInfo("[GreetingPlugin] BAN登録: " + playerName + " 理由: " + reason);
        return true;
    }
    ...
    private boolean handleUnban(CommandSender sender, String[] args) {
        ...
        if ((playerSender == null || !hasPermission(sender, "greetmate.command.greetunban")
                || !hasRequiredRole(playerSender))
                || !validateArgsLength(sender, args, 1, "§e[GreetingPlugin] 使用法: /greetunban <player>"))
            return true;
        ...
        if (deleted) {
            sender.sendMessage("§a[GreetingPlugin] " + playerName + " のBANを解除しました。");
            LogWriter.writeInfo("[GreetingPlugin] BAN解除: " + playerName);
        } else {
            sender.sendMessage("§c[GreetingPlugin] BAN解除に失敗しました。該当プレイヤーは登録されていない可能性があります。");
        }
    ...
    private boolean handleRoleCommand(CommandSender sender, String[] args) {
        ...
        if (!validateArgsLength(sender, args, 1, "§e[GreetingPlugin] 使用法: /greetrole <set|register|del>"))
            return true;
        ...
        switch (args[0].toLowerCase()) {
            ...
            case "del":
                ...
                break;
            default:
                sender.sendMessage("§e[GreetingPlugin] 使用法: /greetrole <set|register|del>");
        }
    ...
    private boolean handleSet(CommandSender sender, String[] args) {
        ...
        if (data == null) {
            sender.sendMessage("§c[GreetingPlugin] 指定されたプレイヤーは登録されていません。/greetrole register を使ってください。");
            return true;
        }

        boolean updated = PlayerRoleDao.updatePlayerRoleByUUID(uuid, roleId);
        if (updated) {
            sender.sendMessage("§a[GreetingPlugin] ロールを更新しました: " + playerName + " → " + roleId);
            LogWriter.writeInfo("[GreetingPlugin] ロール更新: " + playerName + " → " + roleId);
        } else {
            sender.sendMessage("§c[GreetingPlugin] ロールの更新に失敗しました。");
        }
        return true;
    }
    ...
    private void handleRegister(CommandSender sender, String[] args) {
        ...
        if (existing != null) {
            sender.sendMessage("§c[GreetingPlugin] 既に登録されています。/greetrole set を使ってください。");
            return;
        }

        boolean inserted = PlayerRoleDao.insertPlayer(uuid, playerName, roleId);
        if (inserted) {
            sender.sendMessage("§a[GreetingPlugin] プレイヤーを登録しました: " + playerName + "（ロール: " + roleId + "）");
            LogWriter.writeInfo("[GreetingPlugin] 新規プレイヤー登録: " + playerName + "（ロール: " + roleId + "）");
        } else {
            sender.sendMessage("§c[GreetingPlugin] プレイヤー登録に失敗しました。");
        }
    }
    ...
    private void handleDelete(CommandSender sender, String[] args) {
        ...
        if (!isValid) {
            sender.sendMessage("§c[GreetingPlugin] 引数が不正です。");
        } else {
            Player target = getPlayerOrAbort(sender, playerName);
            if (target == null) {
                sender.sendMessage("§c[GreetingPlugin] プレイヤーの取得に失敗しました。");
            } else {
                UUID uuid = target.getUniqueId();
                boolean deleted = PlayerRoleDao.deletePlayerByUUID(uuid);
                if (deleted) {
                    sender.sendMessage("§a[GreetingPlugin] プレイヤー情報を削除しました: " + playerName);
                    LogWriter.writeInfo("[GreetingPlugin] プレイヤー情報削除: " + playerName);
                } else {
                    sender.sendMessage("§c[GreetingPlugin] 削除に失敗しました。");
                }
            }
        }
    }
    ...
    private boolean hasPermission(CommandSender sender, String permission) {
        if (!sender.hasPermission(permission)) {
            sender.sendMessage("§c[GreetingPlugin] 権限がありません。");
            return false;
        }
        return true;
    }
    ...
    private Player validatePlayerSender(CommandSender sender) {
        if (!(sender instanceof Player player)) {
            sender.sendMessage("§c[GreetingPlugin] プレイヤーからのみ実行できます。");
            return null;
        }
        return player;
    }
    ...
    private boolean hasRequiredRole(Player player) {
        ...
        if (data == null || (data.getRole() != 3 && data.getRole() != 4)) {
            player.sendMessage("§c[GreetingPlugin] ロール3または4のユーザーのみ実行可能です。");
            return false;
        }
        return true;
    }
    ...
    private Integer getValidRoleIdOrAbort(CommandSender sender, String input) {
        try {
            ...
            if (!PlayerRoleDao.isValidRole(roleId)) {
                sender.sendMessage("§c[GreetingPlugin] 指定されたロールIDは存在しません。");
                return null;
            }
            sender.sendMessage("§a[GreetingPlugin] ロールIDの検証に成功しました。");
            return roleId;
        } catch (NumberFormatException e) {
            sender.sendMessage("§c[GreetingPlugin] ロールは数値で指定してください。");
            return null;
        }
    }
    ...
    Player getPlayerOrAbort(CommandSender sender, String name) {
        ...
        if (player == null) {
            sender.sendMessage("§c[GreetingPlugin] プレイヤーが見つかりません（オンラインである必要があります）。");
            return null;
        }
        sender.sendMessage("§a[GreetingPlugin] プレイヤーの検証に成功しました。");
        return player;
    }
