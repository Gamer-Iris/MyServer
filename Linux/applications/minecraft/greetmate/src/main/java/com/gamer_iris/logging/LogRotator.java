...
public class LogRotator {
    ...
    private static final String[] TARGET_FILES = { "info.log", "error.log" };
    ...
    public static void rotateLogs() {
        ...
        for (String fileName : TARGET_FILES) {
            ...
            try {
                ...
                Bukkit.getLogger().warning("[greetmate] ログローテーションに失敗: " + e.getMessage());
            }
        }
    ...
    private static void deleteOldLogs(File logDir) {
        File[] files = logDir.listFiles((_, name) -> name.matches("(info|error)_\\d{8}_\\d{6}\\.log"));
        if (files == null)
            return;
