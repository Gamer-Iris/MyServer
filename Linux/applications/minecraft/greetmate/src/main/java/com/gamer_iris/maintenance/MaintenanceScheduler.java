...
public class MaintenanceScheduler {
    ...
    private static void performMaintenance() {
        LogWriter.writeInfo("[GreetingPlugin] 定期メンテナンス処理を開始します...");

        syncOnlinePlayersToDatabase();
        handleBanPlayers();

        LogWriter.writeInfo("[GreetingPlugin] 定期メンテナンス処理が完了しました。");
    }
    ...
    private static void syncOnlinePlayersToDatabase() {
        for (Player player : Bukkit.getOnlinePlayers()) {
            ...
            if (data == null) {
                ...
                LogWriter.writeInfo("[GreetingPlugin] " + name + " をDBに新規登録しました。");
            }

            if (!UserGreetingCacheManager.contains(uuid)) {
                UserGreetingCacheManager.buildAndCache(uuid);
                LogWriter.writeInfo("[GreetingPlugin] " + name + " のキャッシュ情報を投入しました。");
            }
        }
    }
    ...
    private static void handleBanPlayers() {
        ...
        for (BanPlayerData ban : bannedList) {
            ...
            if (wasInDb) {
                LogWriter.writeInfo("[GreetingPlugin] BANプレイヤーをDBとキャッシュから削除: " + name + " (" + uuid + ")");
    ...
    private static void performCleanup() {
        LogWriter.writeInfo("[GreetingPlugin] プレイヤーデータのクリーンアップを開始します...");
        ...
        LogWriter.writeInfo("[GreetingPlugin] クリーンアップ完了。削除件数: " + deletedCount);
    }
