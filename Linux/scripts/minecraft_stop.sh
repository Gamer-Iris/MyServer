...
#----------------------------------------------------------------------------------------------------------------------------------------------------#
# [修正履歴]                            |                                                                                                            #
# V-001      : 2024/11/02               | Gamer-Iris   新規作成                                                                                      #
#                                       |                                                                                                            #
######################################################################################################################################################
...
# エラーメッセージ設定
ERR_MESSAGE_01="サーバー停止に失敗しました。"
ERR_MESSAGE_02="サーバー停止がタイムアウトしました。"
ERR_MESSAGE_03="ロックファイル削除に失敗しました。"
#*****************************************************************************************************************************************************
# 変数エリア
#*****************************************************************************************************************************************************
# ジョブネーム設定
...
#*****************************************************************************************************************************************************
    "STEP010")
      ...
      # EXEC------------------------------------------------------------------------------------------------------------------------------------------
      RESULT=$(kubectl delete deployment -n minecraft minecraft-proxy minecraft-server1 minecraft-server2 && \
               kubectl get pods -n minecraft -l app=minecraft-proxy -o name | xargs -r kubectl delete -n minecraft --now && \
               kubectl get pods -n minecraft -l app=minecraft-server1 -o name | xargs -r kubectl delete -n minecraft --now && \
               kubectl get pods -n minecraft -l app=minecraft-server2 -o name | xargs -r kubectl delete -n minecraft --now)
      # RETURN----------------------------------------------------------------------------------------------------------------------------------------
      RTN_CD=$?
      if [ -n "${RESULT}" ]; then
        log "${RESULT}"
...
#*****************************************************************************************************************************************************
    "STEP030")
      ...
      # EXEC------------------------------------------------------------------------------------------------------------------------------------------
      RESULT=$(find /mnt/share/k8s/minecraft/proxy -name "*.lck" -type f -exec rm {} \; && \
               find /mnt/share/k8s/minecraft/server1 -name "*.lock" -type f -exec rm {} \; && \
               find /mnt/share/k8s/minecraft/server2 -name "*.lock" -type f -exec rm {} \;)
      # RETURN----------------------------------------------------------------------------------------------------------------------------------------
      RTN_CD=$?
      if [ -n "${RESULT}" ]; then
        log "${RESULT}"
